 ...
 * @param {String} token 
 * @param {String} REALM_SID 
 * @param {String} ACCOUNT_SID 
 * @param {String} AUTH_TOKEN 
 * @returns {Promise}  Promise that resolves to true is token is valid, false if not.
 */
const tokenCheck = async (token, REALM_SID, ACCOUNT_SID, AUTH_TOKEN) => {
  try{
    let {data} = await axios.post("https://iam.twilio.com/v2/Tokens/validate/"+REALM_SID,
      {
          token
      },
      {
